...
void RawForceTorqueDataMsgCallBack(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
  ...
  else
  {
    ...
    tool_acc_data(0,0) = msg->data[12];
    tool_acc_data(1,0) = msg->data[13];
    tool_acc_data(2,0) = msg->data[14];
  }
}
...
int main (int argc, char **argv)
{
  ...
  while (!stop)
  {
    ...
    filtered_force_torque_data_msg.data.push_back(tool_estimation->filtered_data(0,0));
    filtered_force_torque_data_msg.data.push_back(tool_estimation->filtered_data(1,0));
    filtered_force_torque_data_msg.data.push_back(tool_estimation->filtered_data(2,0));
